...
<title>醫療新知輪播圖片一覽 - 采姿美管理系統</title>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<!-- title -->
				<h2 class="sub-header">醫療新知輪播圖片一覽</h2>
						...
						<c:forEach var="bean" items="${imageList}" varStatus="status">
							<tbody>
								<tr>
									...
									<td rowspan="4" style="width:25%"><img src="<%=request.getContextPath()%>/images/slider-knowledge/${bean.im_filename}"></td>
						...
						<nav style="text-align:center">
							<ul class="pagination" style="margin:0">
								<c:if test="${currentPage > 1}">
									<li><a href="<%=request.getContextPath()%>/slider-knowledge/list?page=${currentPage - 1}"><span aria-hidden="true">&laquo;</span></a></li>
								</c:if>
								<c:forEach begin="1" end="${pageCount}" varStatus="status">
									<li id="id-li-page-${status.count}"><a href="<%=request.getContextPath()%>/slider-knowledge/list?page=${status.count}">${status.count}</a></li>
								</c:forEach>
								<c:if test="${currentPage < pageCount}">
									<li><a href="<%=request.getContextPath()%>/slider-knowledge/list?page=${currentPage + 1}"><span aria-hidden="true">&raquo;</span></a></li>
								</c:if>
							</ul>
						</nav>
					</c:if>
